...
export function AppHeader() {
    ...
    return (
        <header className="app-header h-20 flex items-center px-6 shrink-0 justify-between sticky top-0 z-30">
            <div className="flex items-center gap-4">
                ...
                <div ref={searchRef} className="relative hidden md:block">
                    ...
                    <input
                        ...
                        className="glass-input pl-10 pr-4 py-2 text-foreground placeholder:text-muted-foreground rounded-xl text-sm transition-all w-64 outline-none focus:ring-1 focus:ring-primary/20"
                    />
                    {searchQuery && (
                    ...
                    {searchOpen && (
                        <div className="glass-overlay absolute top-full left-0 mt-2 w-80 rounded-xl overflow-hidden z-50">
                            ...
                            <div className="border-t border-border px-3 py-2 text-[10px] text-muted-foreground">
                                <kbd className="glass-kbd px-1.5 py-0.5 rounded text-[10px]">Ctrl+K</kbd> para buscar
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div className="flex items-center gap-4">
                    ...
                    <DropdownMenuTrigger asChild>
                        <button aria-label="Notificaciones" className="p-2 text-muted-foreground hover:text-foreground rounded-xl transition-all relative">
                            ...
                            {notifications.length > 0 && (
                                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 rounded-full text-[9px] font-bold text-white flex items-center justify-center border-2 border-background">
                                    {notifications.length}
                                </span>
                            )}
                        </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="glass-overlay w-80 mt-2 rounded-2xl p-2">
                ...
                {user && (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="glass-overlay w-56 mt-2 rounded-2xl p-2">
                ...
                <AlertDialog open={logoutOpen} onOpenChange={setLogoutOpen}>
                    <AlertDialogContent className="glass-overlay rounded-2xl">
                        <AlertDialogHeader>
                        ...
                        <AlertDialogFooter>
                            <AlertDialogCancel className="border-border text-muted-foreground bg-transparent hover:bg-muted/40">Cancelar</AlertDialogCancel>
                            <AlertDialogAction
