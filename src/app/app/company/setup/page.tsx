...
export default function CompanySetupPage() {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[100dvh] p-4 max-w-4xl mx-auto w-full animate-in fade-in duration-700">
            ...
            <div className="w-full relative flex items-center justify-between mb-8 md:mb-12 max-w-2xl px-4">
                ...
                {[1, 2, 3].map((num) => (
                    <div key={num} className="flex flex-col items-center gap-2">
                        ...
                        <span className={`hidden md:block text-xs uppercase font-bold tracking-widest ${step >= num ? 'text-foreground' : 'text-muted-foreground'
                            }`}>
            ...
            <Card className="w-full border-none shadow-2xl bg-card rounded-[32px] overflow-hidden relative">
                ...
                <div className="grid lg:grid-cols-5 min-h-[500px]">
                    ...
                    {/* Right form */}
                    <div className="lg:col-span-3 p-6 md:p-10 bg-card flex flex-col">
                        ...
                        {step === 2 && (
                            <div className="space-y-6 animate-in slide-in-from-right duration-300 flex-1 flex flex-col">
                                <div className="space-y-2 mb-4">
                                    <h3 className="text-2xl font-black text-foreground" style={{ fontFamily: "'Outfit', sans-serif" }}>¿Cómo operas principalmente?</h3>
                                    <p className="text-muted-foreground text-sm">Basado en que eres un negocio de {industries.find(i => i.id === formData.industry)?.label}</p>
                                </div>
                                        ...
                                        <label
                                            ...
                                            className={`
                                                ...
                                                ${formData.actividad === act.label
                                                    ...
                                                    : 'border-border hover:border-emerald-200 hover:bg-muted'
                                                }
                                            `}
                                        >
                                            <div className="flex-1">
                                                <span className={`font-bold text-base ${formData.actividad === act.label ? 'text-emerald-900' : 'text-foreground'}`}>
                                                    {act.label}
                                                </span>
                                            </div>
                                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${formData.actividad === act.label ? 'border-emerald-500 bg-emerald-500' : 'border-border'}`}>
                                ...
                                <div className="flex gap-4 pt-4 mt-auto">
                                    <Button variant="outline" onClick={prevStep} className="h-14 px-6 border-border text-muted-foreground font-bold rounded-2xl hover:bg-muted">Atrás</Button>
                        ...
                        {step === 3 && (
                            <form onSubmit={handleSubmit} className="space-y-6 animate-in slide-in-from-right duration-300 flex-1 flex flex-col">
                                <div className="space-y-2 mb-4">
                                    <h3 className="text-2xl font-black text-foreground" style={{ fontFamily: "'Outfit', sans-serif" }}>¿De qué tamaño es tu equipo?</h3>
                                    <p className="text-muted-foreground text-sm">Nos ayuda a saber si necesitas módulos de colaboración.</p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 flex-1 content-start">
                                    {teamSizes.map((size) => (
                                        <label
                                            ...
                                            `}
                                        >
                                            <span className={`font-bold text-sm ${formData.tamano_equipo === size.label ? 'text-emerald-900' : 'text-foreground'}`}>
                                                {size.label}
                                            </span>
                                            <input
                                                type="radio"
                                ...
                                <div className="flex gap-4 pt-4 mt-auto">
                                    <Button type="button" variant="outline" onClick={prevStep} className="h-14 px-6 border-border text-muted-foreground font-bold rounded-2xl hover:bg-muted">Atrás</Button>
                                    <Button
                                        type="submit"
